...
def log_step(step, action, reward, done, error=None):
    print("[STEP] step=" + str(step) + " action=" + action + " reward=" + str(round(reward,2)) + " done=" + str(done).lower() + " error=" + str(error or "null"), flush=True)

def log_end(success, steps, score, rewards):
    print("[END] success=" + str(success).lower() + " steps=" + str(steps) + " score=" + str(round(score,3)), flush=True)
